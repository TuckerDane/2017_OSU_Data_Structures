//************************************************************************************************************************
//
//	CS261- Assignment 4 - dynamicArray.c
//
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	if (rightIdx < last)
	{
		// check to see if right is the smaller of the two
		if (compare(heap->data[leftIdx], heap->data[rightIdx]) == 1)
		{
			// if it is, compare it to the parent
			if (compare(heap->data[rightIdx], heap->data[position]) == -1)
			{
				// if the right child is smaller than the parent, swap and call adjust heap
				dySwap(heap, rightIdx, position);
				adjustHeap(heap, last, rightIdx, compare);
			}
		}
		else // if it not, then the left is the smaller of the two
		{
			// compare the left to the parent
			if (compare(heap->data[leftIdx], heap->data[position]) == -1)
			{
				// if the left child is smaller than the parent, swap and call adjust heap
				dySwap(heap, leftIdx, position);
				adjustHeap(heap, last, leftIdx, compare);
			}
		}
	}
	else if (leftIdx < last)	// if only one child (the left)
	{
		// have only one child
		//compare child to parent
		if (compare(heap->data[leftIdx], heap->data[position]) == -1)
		{
			//if the child is smaller than the parent, swap and call adjustheap(max, leftIdx)
			dySwap(heap, leftIdx, position);
			adjustHeap(heap, last, leftIdx, compare);
		}		
	}
...
void buildHeap(DynamicArray* heap, compareFunction compare)
{
	...
	while (parent >= 0)
	{
		// swap the parent and percolate down if it is larger than its children (citation for root concept: https://johnderinger.wordpress.com/2012/12/28/heapify/)
		int root = parent;
		while ((root * 2 + 1) <= heap->size - 1)
		{
			int leftChild = root * 2 + 1;
			int rightChild = root * 2 + 2;
			// if position has 2 children
			if (rightChild < heap->size)
			{
				// if the left child is smaller than the right child
				if (compare(heap->data[leftChild], heap->data[rightChild]) == -1)
				{
					// and the left child is smaller than the root
					if (compare(heap->data[leftChild], heap->data[root]) == -1)
					{
						// swap the left child with the root
						dySwap(heap, leftChild, root);
						root = leftChild;
					}
					else // the root is in the right spot, break out of the loop
					{
						break;
					}
				}
				...
				{
					// and if the right child is smaller than the root
					if (compare(heap->data[rightChild], heap->data[root]) == -1)
					{
						// swap the right child with the root
						dySwap(heap, rightChild, root);
						root = rightChild;
					}
					else // the parent is in the right spot, break out of the loop
					{
						break;
					}
				}
			}
			else // position has 1 child
			{
				// if the only(left) child is smaller than the root
				if (compare(heap->data[leftChild], heap->data[root]) == -1)
				{
					dySwap(heap, leftChild, root);
					root = leftChild;
 ...
 * @param compare  pointer to compare function.
 */
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)			// referenced lecture slides: (https://courses.ecampus.oregonstate.edu/index.php?video=cs261/heaps2.mp4)
{
	dyAdd(heap, value);
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
	...
	dyPut(heap, dyGet(heap, last), 0);		// put the last element into the first position
	dyRemoveAt(heap, last);   				// get rid of the last element
	adjustHeap(heap, last, 0, compare);		// adjust the heap
}
...
void dyHeapSort(DynamicArray* heap, compareFunction compare)
{
	...
	while (position != 0)
	{
		dySwap(heap, 0, position);
		adjustHeap(heap, position, 0, compare);
		position--;
	}
}
